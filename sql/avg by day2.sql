...
SELECT utc_timestamp,
       GB_temperature,
       avg(GB_temperature)
           OVER (ORDER BY utc_timestamp ROWS BETWEEN 3 PRECEDING AND 3 FOLLOWING) as MovingAverageWindow7
FROM weather_data
...
SELECT utc_timestamp,
       ...
       (SELECT AVG(GB_temperature)
        ...
        WHERE datetime(t1.utc_timestamp, '-3 days') <= datetime(t2.utc_timestamp)
          AND datetime(t1.utc_timestamp, '+3 days') >= datetime(t2.utc_timestamp)
       ) AS MAVG
FROM weather_data t1
GROUP BY strftime('%Y-%m-%d', utc_timestamp);

SELECT utc_timestamp,
       utc_timestamp - LAG(utc_timestamp, 1, utc_timestamp) OVER (
           ORDER BY utc_timestamp
           ) difference
FROM (
